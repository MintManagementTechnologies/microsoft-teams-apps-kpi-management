...
import { createMicrosoftGraphClient, ErrorWithCode, IdentityType, TeamsFx } from "@microsoft/teamsfx";
	...
	const { data, error, loading, reload } = useData(
		async () => {
			try {
				...
				} else if (err instanceof ErrorWithCode && err.message?.includes("CancelledByUser")) {

...
import {
    useLazyGetMultipleUsersWithPhotoAndPresenceQuery, useLazyGetUserWithPhotoAndPresenceQuery
...
import { useSubmitGoalBatchForApprovalMutation } from '../../goalBatchService';
import { formStateChanged } from '../../goalBatchSlice';

const SubmitGoalBatch = (props: { id: number, userId: number }): JSX.Element => {
   ...
   const [submitGoalBatchForApproval, { isLoading, isError: isErrorSubmitGoalBatch, error: errorSubmitGoalBatch }]
   ...
   useEffect(() => {
      if (!isErrorSubmitGoalBatch) return;
      setError({
         show: true,
         ...
         details: JSON.stringify(errorSubmitGoalBatch)
      });
   }, [isErrorSubmitGoalBatch]);

   const handleOnSubmit = async (event: any) => {
      event.preventDefault();
      submitGoalBatchForApproval({ id: id, userId: userId }).unwrap().then((result: any) => {
   ...
   return (
      <>
         <Flex gap='gap.small' hAlign={'center'} vAlign={'center'} padding="padding.medium" column fill className='mmt-goalBatch-submit mmt-taskModule-headerAndBody'>
            <Flex hAlign={'center'} fill column >
               <br />
               <br />
               <Image className="mmt-goalBatch-submit-image" src="/images/thumbsUp.svg" fluid />
            </Flex>
            <Flex gap='gap.small' hAlign={'center'} column className='mmt-taskModule-header' fill >
               <Text content={`${t('modal.myGoals.goalBatch.title')}`} color={`brand`} size="large" weight="bold" />
               <Text content={`${t('modal.myGoals.goalBatch.description')}`} temporary size="small" />
            </Flex>
         </Flex>
...
export default SubmitGoalBatch;
